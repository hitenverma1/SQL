...
SELECT name, DAY(whn), (confirmed -
   LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY whn)) as new_cases
 FROM covid
WHERE name = 'Italy'
AND MONTH(whn) = 3 AND YEAR(whn) = 2020
ORDER BY whn
...
SELECT tw.name, DATE_FORMAT(tw.whn,'%Y-%m-%d') as dates, 
 tw.confirmed as tc, lw.confirmed as lc
 FROM covid tw LEFT JOIN covid lw ON 
  DATE_ADD(lw.whn, INTERVAL 1 WEEK) = tw.whn
   AND tw.name=lw.name
WHERE WEEKDAY(tw.whn) = 0  and tw.name = 'Italy'
ORDER BY tw.whn)

select name, dates, (tc-lc) as confirmed from cte1
...
with cte1 as(
SELECT name, DATE_FORMAT(whn,'%Y-%m-%d') as dates, (confirmed -
   LAG(confirmed, 1) OVER (PARTITION BY name ORDER BY whn)) as new_cases
 FROM covid
ORDER BY whn)

select name, dates, new_cases from(
select *,
row_number() over(partition by name order by new_cases desc) as rnk
 from cte1
where new_cases > 1000) as q
where rnk = 1
order by dates asc,name desc, new_cases desc
